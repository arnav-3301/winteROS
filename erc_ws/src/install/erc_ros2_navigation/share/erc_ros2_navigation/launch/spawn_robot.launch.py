...
def generate_launch_description():
    ...
    urdf_file_path = PathJoinSubstitution([
        pkg_erc_ros2_navigation,  # Replace with your package name
        "urdf",
    ...
    world_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(pkg_erc_ros2_navigation, 'launch', 'world.launch.py'),
        ),
        launch_arguments={
    ...
    ekf_node = Node(
        ...
        output='screen',
        parameters=[
            os.path.join(pkg_erc_ros2_navigation, 'config', 'ekf.yaml'),
            {'use_sim_time': LaunchConfiguration('use_sim_time')},
             ]
    )
